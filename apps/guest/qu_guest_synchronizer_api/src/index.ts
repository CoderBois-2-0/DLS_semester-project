...
const app = new Hono();
const PORT = parseInt(process.env.PORT || '3040'); // Guest Synchronizer port
const RABBITMQ_URL =
...
async function setupRabbitMQ() {
    try {
        ...
        // Start consumer for Admin data
        await createEventQueue.startConsumer(connection, async (msg) => {
            if (msg) {
                console.log(
                    'Received message from Admin Organiser Backend:',
                    msg.content.toString()
                );
            }
        });

        console.log('Guest Synchronizer is listening for messages...');
    } catch (error) {
...
app.get('/', (c) => c.text('Guest Synchronizer API'));
